...
<html lang="pt-BR" class="dark">
  ...
  <body class="min-h-screen overflow-x-auto bg-ink text-slate-200 antialiased">
    <div class="flex min-h-screen">
      ...
      <main class="min-w-0 flex-1">
        ...
        <section id="dashboard" class="mx-auto max-w-7xl p-5 md:p-9">
          <div class="mb-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><button id="dashboardBack" class="hidden mb-2 text-sm font-medium text-violet-300 hover:text-violet-200">← Voltar ao CRUD pai</button><h2 id="dashboardTitle" class="text-2xl font-bold tracking-tight text-white">Gerencie suas estruturas</h2><p id="dashboardDescription" class="mt-1 text-sm text-slate-500">Crie, organize e registre dados com total flexibilidade.</p></div><div class="flex gap-2"><div class="relative"><span class="absolute left-3 top-2.5 text-slate-500">⌕</span><input id="search" class="w-full rounded-lg border border-line bg-panel py-2 pl-9 pr-3 text-sm outline-none placeholder:text-slate-600 focus:border-violet sm:w-56" placeholder="Buscar CRUD..." /></div><button class="rounded-lg border border-line bg-panel px-3 text-sm text-slate-400">≡ Filtros</button></div></div>
        ...
        <section id="structureDetail" class="hidden mx-auto max-w-7xl p-5 md:p-9">
          ...
          <div class="grid gap-6 lg:grid-cols-[1fr_360px]"><div class="rounded-xl border border-line bg-panel p-4"><h3 class="text-base font-semibold text-white">Colunas deste CRUD</h3><div id="columnsList" class="mt-4 space-y-2"></div></div><aside class="rounded-xl border border-line bg-panel p-4"><form id="columnForm" class="space-y-3"><h3 id="columnFormTitle" class="text-base font-semibold text-white">Adicionar coluna</h3><input required name="name" class="input" placeholder="Nome da coluna" /><select name="type" class="input"><option value="0">Texto</option><option value="1">Número inteiro</option><option value="2">Seleção</option></select><input required name="position" type="number" value="0" class="input" placeholder="Ordem" /><label class="flex cursor-pointer items-center justify-between gap-3 rounded-lg border border-line bg-slate-950/40 px-3 py-2.5 text-sm text-slate-300"><span><span class="block font-medium">Permitir valores repetidos</span><small class="mt-0.5 block text-xs text-slate-500">Desative para exigir um valor único nesta coluna.</small></span><input id="allowDuplicates" name="allow_duplicates" type="checkbox" role="switch" checked class="peer sr-only" /><span aria-hidden="true" class="relative h-6 w-11 shrink-0 rounded-full bg-slate-700 transition peer-checked:bg-violet peer-focus:ring-2 peer-focus:ring-violet after:absolute after:left-1 after:top-1 after:h-4 after:w-4 after:rounded-full after:bg-white after:transition peer-checked:after:translate-x-5"></span></label><div class="flex gap-2"><button type="button" id="cancelColumnEdit" class="hidden rounded-lg px-3 py-2 text-sm text-slate-400 hover:text-white">Cancelar</button><button id="saveColumn" class="flex-1 rounded-lg border border-violet/60 px-3 py-2 text-sm font-semibold text-violet-200 hover:bg-violet/10">Adicionar coluna</button></div></form></aside></div>
        </section>
    ...
    <div id="modal" class="fixed inset-0 z-20 hidden items-center justify-center bg-slate-950/75 p-4 backdrop-blur-sm">
      <form id="crudForm" class="w-full max-w-lg rounded-2xl border border-line bg-[#11182a] p-6 shadow-2xl"><div class="mb-6 flex items-start justify-between"><div><h2 id="crudModalTitle" class="text-xl font-bold text-white">Criar novo CRUD</h2><p id="crudModalDescription" class="mt-1 text-sm text-slate-500">Configure a estrutura inicial do seu cadastro.</p></div><button type="button" id="closeModal" aria-label="Fechar modal" class="text-xl text-slate-500 hover:text-white">×</button></div><label class="block text-sm font-medium text-slate-300">Nome do CRUD<input required id="crudName" class="mt-2 w-full rounded-lg border border-line bg-[#0b1020] px-3 py-2.5 outline-none focus:border-violet" placeholder="Ex.: Fornecedores" /></label><fieldset class="mt-5"><legend class="text-sm font-medium text-slate-300">Orientação das colunas</legend><div class="mt-2 grid grid-cols-2 gap-3"><label class="cursor-pointer rounded-lg border border-violet bg-violet/10 p-3"><input checked type="radio" name="orientation" value="0" class="accent-violet" /> <span class="ml-1 text-sm font-medium">Horizontal</span><small class="mt-1 block text-xs text-slate-500">Registros em linhas</small></label><label class="cursor-pointer rounded-lg border border-line p-3"><input type="radio" name="orientation" value="1" class="accent-violet" /> <span class="ml-1 text-sm font-medium">Vertical</span><small class="mt-1 block text-xs text-slate-500">Registros em colunas</small></label></div></fieldset><fieldset class="mt-5"><legend class="text-sm font-medium text-slate-300">Tipo do CRUD</legend><div class="mt-2 grid grid-cols-2 gap-3"><label class="cursor-pointer rounded-lg border border-violet bg-violet/10 p-3"><input checked type="radio" name="crudType" value="0" class="accent-violet" /> <span class="ml-1 text-sm font-medium">CRUD simples</span></label><label class="cursor-pointer rounded-lg border border-line p-3"><input type="radio" name="crudType" value="1" class="accent-violet" /> <span class="ml-1 text-sm font-medium">CRUD de CRUDs</span></label></div></fieldset><div class="mt-6 flex items-center justify-between gap-3"><button type="button" id="deleteCrud" class="hidden rounded-lg border border-rose-500/50 px-4 py-2 text-sm font-semibold text-rose-300 hover:bg-rose-500/10">Excluir CRUD</button><div class="ml-auto flex gap-3"><button type="button" id="cancel" class="rounded-lg px-4 py-2 text-sm text-slate-400">Cancelar</button><button id="saveCrud" class="rounded-lg bg-violet px-4 py-2 text-sm font-semibold text-white">Criar estrutura</button></div></div></form>
    </div>
    ...
    <div id="alertModal" class="fixed inset-0 z-40 hidden items-center justify-center bg-slate-950/75 p-4 backdrop-blur-sm" role="alertdialog" aria-modal="true" aria-labelledby="alertModalTitle"><div class="w-full max-w-md rounded-2xl border border-rose-500/40 bg-panel p-6 shadow-2xl"><div class="flex items-start gap-3"><span class="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-rose-500/15 text-lg text-rose-300" aria-hidden="true">!</span><div><h2 id="alertModalTitle" class="text-xl font-bold text-white">Não foi possível salvar</h2><p id="alertModalMessage" class="mt-2 text-sm leading-6 text-slate-300"></p></div></div><div class="mt-6 flex justify-end"><button id="closeAlertModal" type="button" class="rounded-lg bg-violet px-4 py-2 text-sm font-semibold text-white hover:bg-violet-500">Entendi</button></div></div></div>
    <style>
